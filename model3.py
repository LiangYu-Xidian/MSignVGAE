...
class VGAE_dot_conv_leakyrelu(nn.Module):
    def __init__(self):
        ...
        self.conv1 = GraphConv(args.input_dim_hmdad_sign, args.hidden1_dim, activation=F.leaky_relu)
        self.conv2 = GraphConv(args.hidden1_dim, args.hidden2_dim, activation=F.leaky_relu)

        self.meanGCN1 = GraphConv(args.hidden2_dim, args.z1, activation=lambda x: x)
        self.logstdGCN1 = GraphConv(args.hidden2_dim, args.z1, activation=lambda x: x)

        self.meanGCN2 = GraphConv(args.hidden2_dim, args.z2, activation=lambda x: x)
        self.logstdGCN2 = GraphConv(args.hidden2_dim, args.z2, activation=lambda x: x)

        self.meanGCN3 = GraphConv(args.hidden2_dim, args.z3, activation=lambda x: x)
        self.logstdGCN3 = GraphConv(args.hidden2_dim, args.z3, activation=lambda x: x)

        self.transform_z = nn.Linear(args.z1 + args.z2 + args.z3, args.z1 + args.z2 + args.z3)
        self.dropout = nn.Dropout(0.5)
        self.relu = nn.LeakyReLU(0.1)

        self.aux1 = nn.Linear(args.z1 + args.z2+ args.z3, args.hidden1_dim)
        self.dropout1 = nn.Dropout(0.5)
        self.relu1 = nn.LeakyReLU(0.1)
        # self.relu1 = nn.ReLU()

        self.aux2 = nn.Linear(args.z1 + args.z2 + args.z3, args.hidden2_dim)
        self.dropout2 = nn.Dropout(0.5)
        self.relu2 = nn.LeakyReLU(0.1)
        # self.relu2 = nn.ReLU()
