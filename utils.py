...
def get_all_the_samples_sign(A):
    m, n = A.shape
    pos = []
    neg = []
    for i in range(m):
...
def preprocess_graph_signGCN(adj):
    adj_unsign = np.where(adj==0, 0, 1)
    adj_unsign = sp.coo_matrix(adj_unsign)

    adj_sign = sp.coo_matrix(adj)
    # adj_sign = adj_sign + sp.eye(adj_sign.shape[0])
    
    # adj_ = adj_unsign + sp.eye(adj_unsign.shape[0])
    rowsum = np.array(adj_unsign.sum(1))
    degree_mat_inv_sqrt = sp.diags(np.power(rowsum, -0.5).flatten())
    adj_normalized = adj_sign.dot(degree_mat_inv_sqrt).transpose().dot(degree_mat_inv_sqrt).tocoo()
    return sparse_to_tuple(adj_normalized)

def preprocess_graph_no_signGCN(adj):
    adj_unsign = np.where(adj==0, 0, 1)
    adj_unsign = sp.coo_matrix(adj_unsign)
    ...
    rowsum = np.array(adj_unsign.sum(1))
    degree_mat_inv_sqrt = sp.diags(np.power(rowsum, -0.5).flatten())
    adj_normalized = adj_unsign.dot(degree_mat_inv_sqrt).transpose().dot(degree_mat_inv_sqrt).tocoo()
    return sparse_to_tuple(adj_normalized)
...
def get_features_A(A):
    mat1 = np.where(A == 1, 1, 0)
    mat2 = np.where(A == -1, -1, 0)

    features_m = np.hstack((mat1, mat2))
    features_d = np.hstack((mat1.T, mat2.T))
    return features_m, features_d


def draw_PR_multiclass(folds, title=None, savePath=None, dpi=300):
    '''
    :param folds:
    :param title:
    :param savePath:
