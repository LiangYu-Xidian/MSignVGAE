...
sim_m, sim_d = get_sim_3ss_1ms_phendb(A, k1, k2)
...
for i in range(len(w_aux1s)):
    for j in range(len(w_aux2s)):
        ...
        suffix = '3ss_1ms_ww1.0_{:.2f}_{:.2f}_{:.2f}'.format(w_aux1s[i], w_aux2s[j], w_w)
        # suffix = '3ss_1ms_all_1_w_weight_{:.2f}_{:.2f}_{:.2f}'.format(w_aux1s[i], w_aux2s[j], w_w)
        ##########################################################################################
        # 对微生物相似性
        ...
        for epoch in range(5000):
            ...
            RMSE = np.sqrt(mean_squared_error(adj_label.cpu().detach().numpy().reshape(-1),
                                              A_pred.cpu().detach().numpy().reshape(-1)))
            tra_r2.append(r2)
            ...
            if RMSE < min_mse:
                ...
                torch.save(state, './phendb/opt/models/M_mse_256_128_64_5000_' + suffix + '.pth')
        ...
        np.save('./phendb/opt/metrics/M_mse_256_128_64_5000_' + suffix + '.npy', save_dic)
        model_ = VGAE3()
        model_.to(device)
        model_.load_state_dict(torch.load('./phendb/opt/models/M_mse_256_128_64_5000_' + suffix + '.pth')['model'])
        model_.eval()
        out_m = model_(adj_norm, features)

        latent_m = out_m[-1].cpu().detach().numpy()
        np.save('./phendb/opt/latent/latent_m_mse_' + suffix + '.npy', latent_m)
        ...
        for epoch in range(5000):
            ...
            RMSE = np.sqrt(mean_squared_error(adj_label2.cpu().detach().numpy().reshape(-1),
                                              A_pred.cpu().detach().numpy().reshape(-1)))
            tra_r2.append(r2)
            tra_RMSE.append(RMSE)
            ...
            if base_loss.item() < min_loss:
                ...
                torch.save(state, './phendb/opt/models/D_loss_512_256_128_5000_' + suffix + '.pth')
        model2.eval()
        ...
        np.save('./phendb/opt/metrics/D_loss_512_256_128_5000_' + suffix + '.npy', save_dic)

        model2_ = VGAE4()
        model2_.to(device)
        model2_.load_state_dict(torch.load('./phendb/opt/models/D_loss_512_256_128_5000_' + suffix + '.pth')['model'])
        model2_.eval()
        out_d = model2_(adj_norm2, features2)

        latent_d = out_d[-1].cpu().detach().numpy()
        np.save('./phendb/opt/latent/latent_d_loss_' + suffix + '.npy', latent_d)
