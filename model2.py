...
class VGAE(nn.Module):
    def __init__(self, bias=False):
        ...
        self.conv1 = GraphConv(args.input_dim, args.hidden1_dim, activation=F.relu)
        self.meanGCN = GraphConv(args.hidden1_dim, args.hidden2_dim, activation=lambda x: x)

        self.logstdGCN = GraphConv(args.hidden1_dim, args.hidden2_dim, activation=lambda x: x)

        self.transform_z = nn.Linear(args.hidden2_dim, args.hidden2_dim)
        self.dropout = nn.Dropout(0.5)
        self.relu = nn.ReLU()

    def encoder(self, adj, x):
        x = self.conv1(adj, x)
        self.mean = h = self.meanGCN(adj, x)
        self.logstd = std = self.logstdGCN(adj, x)
        return h, std

    def decoder1(self, x):
        pred = torch.sigmoid(torch.matmul(x, x.t()))
        return pred
...
class VGAE1(nn.Module):
    def __init__(self, bias=False):
        ...
        self.auxi = nn.Linear(args.hidden2_dim, args.hidden1_dim)
    ...
    def forward(self, adj, x):
        mu, logstd, x1 = self.encoder(adj, x)
        z = self.reparameterize(mu, logstd)
        reconst = self.decoder_main(z)
        auxi = self.decoder_2(z)
        return reconst, auxi, x1


class VGAE2(nn.Module):
    def __init__(self):
        super(VGAE2, self).__init__()
        self.conv1 = GraphConv(args.input_dim, args.h1, activation=F.relu)
    ...
    def forward(self, adj, x):
        ...
        return reconst, aux1, aux2, x1, x2
