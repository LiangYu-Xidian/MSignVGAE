...
w_aux1 = 0.
w_aux2 = 0.
w_ws = np.arange(0.1, 1.1, 0.1)
for w_w in w_ws:
    suffix = 'all_w_1-weight_del_aux12_ww{:.2f}'.format(w_w)
    # suffix = 'base_1z_kl'
    ##########################################################################################
    # 对微生物相似性
    ...
    for epoch in range(5000):
        ...
        #              'min_loss': min_loss,
        #              }
        #     torch.save(state, 'models1/M_256_128_64_5000_base_W.pth')
        if RMSE < min_mse:
            min_mse = RMSE
            state = {'model': model.state_dict(),
                     'epoch': epoch,
            ...
            torch.save(state, './opt/models1/M_mse_256_128_64_5000_' + suffix + '.pth')
    ...
    np.save('./opt/metrics1/M_mse_256_128_64_5000_' + suffix + '.npy', save_dic)
    model_ = VGAE3()
    model_.to(device)
    model_.load_state_dict(torch.load('./opt/models1/M_mse_256_128_64_5000_' + suffix + '.pth')['model'])
    model_.eval()
    out_m = model_(adj_norm, features)

    latent_m = out_m[-1].cpu().detach().numpy()
    np.save('./opt/latent1/latent_m_mse_' + suffix + '.npy', latent_m)
    ...
    for epoch in range(5000):
        ...
        if base_loss.item() < min_loss:
            ...
            torch.save(state, './opt/models1/D_loss_256_128_64_5000_' + suffix + '.pth')
        # if RMSE < min_mse:
        #     min_mse = RMSE
        #     state = {'model': model.state_dict(),
        #              'epoch': epoch,
        #              'min_mse': min_mse,
        #              }
        #     torch.save(state, 'models1/D_mse_256_128_64_5000_base_w.pth')
    ...
    np.save('./opt/metrics1/D_loss_256_128_64_5000_' + suffix + '.npy', save_dic)

    model2_ = VGAE4()
    model2_.to(device)
    model2_.load_state_dict(torch.load('./opt/models1/D_loss_256_128_64_5000_' + suffix + '.pth')['model'])
    model2_.eval()
    out_d = model2_(adj_norm2, features2)

    latent_d = out_d[-1].cpu().detach().numpy()
    np.save('./opt/latent1/latent_d_loss_' + suffix + '.npy', latent_d)
