...
# random N(0,1) feature
A = random_feature = np.random.standard_normal(A.shape)
...
for i in range(len(w_aux1s)):
    for j in range(len(w_aux2s)):
        ...
        suffix = '3ss_del_A_init_ms_ww1.0_{:.2f}_{:.2f}_{:.2f}'.format(w_aux1s[i], w_aux2s[j], w_w)
        ...
        for epoch in range(5000):
            ...
            if RMSE < min_mse:
                ...
                torch.save(state, './ablation/models/M_mse_256_128_64_5000_' + suffix + '.pth')
        ...
        np.save('./ablation/metrics/M_mse_256_128_64_5000_' + suffix + '.npy', save_dic)
        model_ = VGAE3()
        model_.to(device)
        model_.load_state_dict(torch.load('./ablation/models/M_mse_256_128_64_5000_' + suffix + '.pth')['model'])
        model_.eval()
        out_m = model_(adj_norm, features)

        latent_m = out_m[-1].cpu().detach().numpy()
        np.save('./ablation/latent/latent_m_mse_' + suffix + '.npy', latent_m)
        ...
        for epoch in range(5000):
            ...
            if base_loss.item() < min_loss:
                ...
                torch.save(state, './ablation/models/D_loss_256_128_64_5000_' + suffix + '.pth')
        model2.eval()
        ...
        np.save('./ablation/metrics/D_loss_256_128_64_5000_' + suffix + '.npy', save_dic)

        model2_ = VGAE4()
        model2_.to(device)
        model2_.load_state_dict(torch.load('./ablation/models/D_loss_256_128_64_5000_' + suffix + '.pth')['model'])
        model2_.eval()
        out_d = model2_(adj_norm2, features2)

        latent_d = out_d[-1].cpu().detach().numpy()
        np.save('./ablation/latent/latent_d_loss_' + suffix + '.npy', latent_d)
