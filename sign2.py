...
# sim_m, sim_d = get_sim_3ss_1ms_disbiome(A, k1, k2)
# sim_m, sim_d = get_3ss_GIP_dis_drug(A, k1, k2)
sim_m, sim_d = np.load('./phendb/sim_m_sign.npy'), np.load('./phendb/sim_d_sign.npy')
...
for epoch in range(10000):
    ...
    if RMSE < min_mse:
        ...
        torch.save(state, './sign/models/M_mse_128_64_32_' + suffix + '.pth')
...
np.save('./sign/metrics/M_mse_128_64_32_' + suffix + '.npy', save_dic)
model_ = VGAE_dot_conv_leakyrelu()
model_.to(device)
model_.load_state_dict(torch.load('./sign/models/M_mse_128_64_32_' + suffix + '.pth')['model'])
model_.eval()
out_m = model_(adj_norm, features)

latent_m = out_m[-1].cpu().detach().numpy()
np.save('./sign/latent_database/latent_' + suffix + '.npy', latent_m)
