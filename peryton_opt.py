...
k1 = 140  # mic
k2 = 5  # dis
A = np.load('./peryton/mic_dis association.npy')
    ...
    for j in range(len(w_aux2s)):
        ...
        for epoch in range(5000):
            ...
            if RMSE < min_mse:
                ...
                torch.save(state, './peryton/opt/models/M_mse_256_128_64_5000_' + suffix + '.pth')
        ...
        np.save('./peryton/opt/metrics/M_mse_256_128_64_5000_' + suffix + '.npy', save_dic)
        model_ = VGAE3()
        model_.to(device)
        model_.load_state_dict(torch.load('./peryton/opt/models/M_mse_256_128_64_5000_' + suffix + '.pth')['model'])
        model_.eval()
        out_m = model_(adj_norm, features)

        latent_m = out_m[-1].cpu().detach().numpy()
        np.save('./peryton/opt/latent/latent_m_mse_' + suffix + '.npy', latent_m)
        ...
        for epoch in range(5000):
            ...
            if base_loss.item() < min_loss:
                ...
                torch.save(state, './peryton/opt/models/D_loss_512_256_128_5000_' + suffix + '.pth')
        model2.eval()
        ...
        np.save('./peryton/opt/metrics/D_loss_512_256_128_5000_' + suffix + '.npy', save_dic)

        model2_ = VGAE4()
        model2_.to(device)
        model2_.load_state_dict(torch.load('./peryton/opt/models/D_loss_512_256_128_5000_' + suffix + '.pth')['model'])
        model2_.eval()
        out_d = model2_(adj_norm2, features2)

        latent_d = out_d[-1].cpu().detach().numpy()
        np.save('./peryton/opt/latent/latent_d_loss_' + suffix + '.npy', latent_d)
